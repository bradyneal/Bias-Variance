...
for corr in corr_list:
    ...
    mean = torch.mean(outputs_corrupt, 0)
    variance = calculate_variance(outputs_corrupt, mean)
    ...
    mean = torch.mean(outputs_normal, 0)
    variance = calculate_variance(outputs_normal, mean)
    variance = torch.Tensor([variance]).unsqueeze(0)
    relative_error = np.sqrt(2 / num_seeds)
    std_variance = variance * relative_error
...
#np.save('to_plot', variances_by_corr_normal)
