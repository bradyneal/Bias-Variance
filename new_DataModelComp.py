...
class DataModelComp:
    ...
    def __init__(self, model, batch_size=100, test_batch_size=10000, epochs=10,
                 ...
                 run_i=0, save_interval=None, data='MNIST'):
        ''' data can be either MNIST or CIFAR10
        '''
        ...
        # Load data
        if data == 'MNIST':
            self.train_loader, self.test_loader = self.get_MNIST_data_loaders()
        if data == 'CIFAR10':
            self.train_loader, self.test_loader = self.get_CIFAR10_data_loaders()
    ...
    def get_MNIST_data_loaders(self, same_dist=False, split_random_seed=0):
    ...
    def get_CIFAR10_data_loaders(self, same_dist=False, split_random_seed=0):
        kwargs = {'num_workers': 1, 'pin_memory': True} if self.cuda else {}
        ...
        transform = transforms.Compose(
            ...
            [transforms.CenterCrop((28, 28)),                   # crop input to 28x28
             ...
             transforms.Lambda(lambda x: (x-x.mean())/x.std())  # per_image_whitening function in TENSORFLOW (Abadi et al., 2015)
             ])
